 ...
 * instances where a waveform is "clipped" by exceeding the range of an ADC, and
 * a qualitative estimation the waveform's original shape is desired.
 * <p>
...
public class InterpolateClippedRegions implements ExtendedPlugInFilter, DialogListener
{
	...
	public void run(ImageProcessor ip)
	{
		...
		double[] interpolatedPixels = execute(pixelsDouble, width, clippingThreshold);

		if (interpolatedPixels != null) {
			for (int i = 0; i < pixels.length; i++) {
				pixels[i] = (float)interpolatedPixels[i];
			}
		}
	}
